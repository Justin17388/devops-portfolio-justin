...
app.config["MYSQL_DATABASE_PASSWORD"] = "Justin_1"
...
def insert_person(name, number):
    ...
    if (
        row is not None
    ):  # If the row is not none, it means, I have a row that has same name given by a user, We'll return user with a massage
...
def update_person(name, number):
    ...
    if (
        row is None
    ):  # First we need to control if there is any person with the same name into our database. if we don't have, a warning massage will raise
...
def delete_person(name):
    ...
    if (
        row is None
    ):  # Again we need to control if we have this person. then, If we don't have, there is seen a warning massage like this
...
def find_records():
    if request.method == "POST":
        ...
        return render_template("index.html", show_result=False, developer_name="Justin")
...
def add_record():
    if request.method == "POST":
        ...
        if name is None or name.strip() == "":
            ...
            name.isdecimal()
        ):  # This will check if the name given by user has any decimal character. If it has, a warning massage will raise
            return render_template(
                "add-update.html",
                not_valid=True,
        ...
        if (
            phone_number is None or phone_number.strip() == ""
...
def delete_record():
    if request.method == "POST":
        ...
        return render_template(
            "delete.html", show_result=False, not_valid=False, developer_name="Justin"
        )
